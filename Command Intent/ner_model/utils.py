...
def convert_annotations_to_csv(file_path):
    ...
    with open(file_path, 'r') as file:
        ...
        variable_declaration = annotations['variable declaration']

        with open("./ner_dataset/ner_dataset.csv", 'w') as csv_file:
            csv_file.write("Sentence #, Word, Tag, Intent\n")

            for sentence_index, example in enumerate(variable_declaration):
                sentence = example[0]
...
def read_dataset():
    ...
    grouped_data = data.groupby('Sentence #').agg({
        ...
        'Intent': lambda x: list(x.str.strip().replace('_', ' '))     # Collect intents into a list
